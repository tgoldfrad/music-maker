...
//    }
//}
using System.Runtime.InteropServices; // הוספה בראש הקובץ
using AutoMapper;
...
namespace Music.Service
{
    ...
    public class PdfToImageRenderer
    {
        // Convert PDF to image using ghostscript command-line tool (cross-platform)
        public static string RenderPdfPageToImage(string pdfFilePath, string outputImagePath, int pageNumber = 1, int dpi = 300)
        {
            //string gsExecutable = RuntimeInformation.IsOSPlatform(OSPlatform.Windows) ? "gswin64c.exe" : "gs";
            ...
            {
                string args = $"-dNOPAUSE -dBATCH -sDEVICE=pngalpha -r{dpi} -dFirstPage={pageNumber} -dLastPage={pageNumber} -sOutputFile=\"{outputImagePath}\" \"{pdfFilePath}\"";
    ...
    public class AudiverisWrapper
    {
        ...
        public static void ConvertToMidi(string inputXmlPath, string outputMidiPath)
        {
            string musescoreExecutable = RuntimeInformation.IsOSPlatform(OSPlatform.Windows)
                ? @"..\MuseScore 3\bin\MuseScore3.exe"
                : "musescore3";

            var process = new Process();
            process.StartInfo.FileName = musescoreExecutable;
            process.StartInfo.Arguments = $"\"{inputXmlPath}\" -o \"{outputMidiPath}\"";
            process.StartInfo.UseShellExecute = false;
            process.Start();
            process.WaitForExit();
        ...
        public static List<byte> ConvertMidiToWav(string midiPath, string wavPath)
        {
            ...
            string fluidsynthExe = RuntimeInformation.IsOSPlatform(OSPlatform.Windows)
